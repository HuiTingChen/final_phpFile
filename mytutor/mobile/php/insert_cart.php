...
$sqlinsert = "SELECT * FROM tbl_carts WHERE user_email = '$email' AND subject_id = '$subjectid' AND cart_status IS NULL";
...
if ($number_of_result > 0) {
	...
	$cartqty = $cartqty + 1;
	$updatecart = "UPDATE `tbl_carts` SET `cart_qty`= '$cartqty' WHERE user_email = '$email' AND subject_id = '$subjectid' AND cart_status IS NULL";
	$conn->query($updatecart);
} else{
    $addcart = "INSERT INTO `tbl_carts`(`user_email`, `subject_id`, `cart_qty`) VALUES ('$email','$subjectid', '$cartqty')";
